/*eslint-disable*/
  export function WebVTTParser() {
    this.parse = function(inputText, mode) {
      var input = convertSpecialChar(inputText)
      //XXX need global search and replace for \0
      var NEWLINE = /\r\n|\r|\n/
      var startTime = Date.now()
      var linePos = 0
      var lines = input.trim().split(NEWLINE)
      ...
      if (lines.length <= 1) {
        var singleLineError = [{
          col: 1,
          line: 1,
          message: ` This VTT is not formatted correctly. All text appears on a single line, which is invalid. <br /> For information on creating and formatting a VTT, please use
          <a style='text-decoration: none; color: #0000EE' target='_blank' href='https://www.w3.org/TR/webvtt1/#introduction-caption'> this page</a>`
        }]
        return {cues:cues, errors:singleLineError, time:Date.now()-startTime, vttText: lines}
      }
      ...
      /* Loop through Text after Header */
      ...
      /* END */
      return {cues:cues, errors:errors, time:Date.now()-startTime, vttText: lines}
    }
  }
  ...
  function convertSpecialChar(mystring) {
    return mystring.replace(/&/g, "&amp;").replace(/\xE2\x80\x94/g, '-').replace(/\xE2\x80\x93/g, '-').replace(/\s->\s/, ' --> ')
  }
